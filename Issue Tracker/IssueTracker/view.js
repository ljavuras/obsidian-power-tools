...
const default_options = {
  ...
  project_name: user_options?.project_note
    ? user_options.project_note
    : getCurrentFolderName(),

  /** Fallback for project note, use project name, or folder name if not supplied */
  project_note: user_options?.project_name
...
function getCurrentFolderName() {
  ...
  return (
    getCurrentFolderPath()?.match(/\/([^\/]+)$/)?.[1] || getCurrentFolderPath()
  );
}

function resolvePath(basepath, relative_path) {
  return obsidian.normalizePath(`${basepath}/${relative_path}`);
}
...
function getFile(filePath) {
  ...
  if (!file instanceof obsidian.TFile) {
    new obsidian.Notice(`${filePath} is not a file.`);
  }
...
async function openFile(file, mode = "current") {
  ...
  // file type check
  if (!(file instanceof obsidian.TFile) && typeof file != "string") {
    new obsidian.Notice(
...
function existsFolder(folderPath) {
  ...
  return (
    app.vault.getAbstractFileByPath(folderPath) instanceof obsidian.TFolder
  );
}
...
const templater = {
  ...
  settings: {
    ...
    folder: app.plugins.plugins["templater-obsidian"].settings.templates_folder,
  },
...
class IssueTracker {
    ...
    this.config = {
      ...
      project: {
        ...
        link: getFileByName(options.project_note)
          ? dv
              .page(getFileByName(options.project_note)?.path)
              ?.file.link.withDisplay(options.project_name)
  ...
  Filter = class {
    ...
    async getStatusCount(status) {
      ...
      return (
        await this.queryIssues(
          this.issueTracker.issueList.issues,
          query_without_status
        )
      ).filter((issue) => issue.status == status).length;
    }
    ...
    async queryIssues(issues, query) {
      ...
      if (query.content.length) {
        // Skip if query doesn't query content
        for (let issue of issues) {
          if (!issue.content) {
            // Cache file content, stript frontmatter
            issue.content = (await app.vault.cachedRead(issue.file)).replace(
              /^---\n([\s\S]*?\n|)---(\n|$)/,
              ""
            );
          }
      ...
      return issues.filter((issue) => {
        if (query?.status) {
          if (issue.status != this.query.status) {
            return false;
          }
        }
        ...
        for (let content of query.content) {
          if (
            !issue.name.toLowerCase().includes(content.toLowerCase()) &&
    ...
    async filterIssues(issues) {
      return (await this.queryIssues(issues, this.query)).sort(
        (issue) => issue,
        "asc",
        (issue1, issue2) => {
          switch (this.sortBy) {
  ...
  SearchBar = class {
    ...
    render(containerEl) {
      ...
      this.newIssueBtn.onclick = () => {
        new this.issueTracker.CreateIssueModal(this.issueTracker).open();
      };
      ...
      constructor(issueTracker) {
        ...
        app.fileManager.processFrontMatter(file, (frontmatter) => {
          ...
          if (
            ...
            frontmatter["searchDate"].length > 0
          ) {
  ...
  IssueList = class {
    ...
    async refresh() {
      ...
      for (const issue of await this.issueTracker.filter.filterIssues(
        this.issues
      )) {
    ...
    noIssuesMessage = {
      noFolder: (containerEl) => {
        ...
        let createFolderBtn = msgEl.createEl("button", {
          text: "Create Folder",
        });
        msgEl.appendChild(new Text(" at "));
        msgEl.createEl("code", { text: this.issueFolder });
        msgEl.appendChild(
          new Text(", or specify your preferred subfolder for issue notes:")
        );
        msgEl.createEl("pre").createEl("code", {
          text:
            'dv.view("path/to/IssueTracker", {\n' +
            "    obsidian: obisidian,\n" +
    ...
    ToolBar = class {
      constructor(issueTracker) {
        this.issueTracker = issueTracker;
        this.statusOpenBtn = new this.StatusBtn(
          this.issueTracker,
          "open",
          getIcon.open()
        );
        this.statusClosedBtn = new this.StatusBtn(
          this.issueTracker,
          "closed",
          getIcon.closed()
        );
      ...
      StatusBtn = class {
        ...
        async refresh() {
          this.countText.nodeValue =
            await this.issueTracker.filter.getStatusCount(this.status);
        }
      };
    };
    ...
    Issue = class {
      ...
      render(containerEl) {
        ...
        issueBodyEl.createDiv({
          cls: "issue-desc",
          text: `#${this.issueNo} opened on ${this.ctime.toFormat(
            "y/MM/dd, hh:mm a"
          )}`,
        });
      }
  ...
  CreateIssueModal = class extends obsidian.Modal {
    ...
    onOpen() {
      ...
      new obsidian.Setting(this.contentEl).addButton((submitBtn) => {
        submitBtn
    ...
    async submit() {
      ...
      if (!newIssueInfo.title) {
        this.titleInput.tooltip.show("Issue title cannot be empty");
        return;
        ...
        this.issueTracker.issueList.issues.name.includes(newIssueInfo.title)
      ) {
        this.titleInput.tooltip.show(`Issue already exists`);
        return;
      }
...
class IssueInfoExporter {
  constructor(newIssueInfo) {
    ...
    if (window.newIssueInfo) {
      clearTimeout(window.newIssueInfo.timeoutID);
    }
   ...
   */
  get issueTrackerLink() {
    return this.issueTrackerConfig.issueTracker.link;
  }
  get projectNoteLink() {
    return this.issueTrackerConfig.project.link;
  }
}
