...
function getFile(filePath) {
  ...
  if (!file instanceof obsidian.TFile) {
    customJS.Error.log(`${filePath} is not a file.`);
  }
...
class StatusDisplay {
  constructor(containerEl) {
    ...
    this.el.appendChild(
      ...
      new Text(
        issueInfo.status.charAt(0).toUpperCase() + issueInfo.status.slice(1)
      )
    );
  }
}
...
widgetsEl.createDiv({
  cls: "issue-info",
  text: `#${issueInfo.issueNo} opened ${issueInfo.ctime.toRelativeCalendar({
    locale: "en",
  })}`,
});
